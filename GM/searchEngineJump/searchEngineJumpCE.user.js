...
(function() {
	...
	var rules = [ //根据规则把搜索引擎列表插入到指定网站		
		...
		// 下载
		{
			...
			name: "Nyaapantsu",
			url: /^https?:\/\/(nyaa\.|www\.|)?(pantsu\.cat|nyoo\.moe|nyaa\.pt)\/search\?/,
			engineList: "download",
		}, {
			name: "Nayapantsu(sukebei)",
			url: /^https?:\/\/sukebei\.(pantsu\.cat|nyoo\.moe|nyaa\.pt)\/search\?/,
			engineList: "download",
		}, {
			name: "shana project",
			...
			name: "SoCiLi",
			url: /^https?:\/\/www\.sobt\.red\/search\//,
			engineList: "download",
		}, {
			name: "Anisearch",
			url: /^https?:\/\/anisearch\.ru\/torrents\?search/,
			engineList: "download",
		}, {
			name: "AniRena",
			...
			name: "age动漫",
			url: /^https?:\/\/bt\.agefans\.com\/search/,
			engineList: "download",
			keyword: "#top_search_input",
		}, {
			name: "xiaohx",
