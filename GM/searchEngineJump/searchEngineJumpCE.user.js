...
(function(){
	...
	var rules = [// 根据规则把搜索引擎列表插入到指定网站
		...
		// /////////////第一个可以当模板看
		{
			...
			enabled: true,// 是否启用
			url: /^https?:\/\/(www|encrypted)\.google\.(?!co\.jp)[^\/]{2,9}\/(webhp|search|#|$|\?)(?!.*tbm=)/,// 在哪个网站上加载,正则.
			mutationTitle: true,	// 是否要监视标题的变化
		...
		},
		{
			name: "google.co.jp",
			enabled: true,
			url: /^https?:\/\/(www|encrypted)\.google\.co\.jp\/(webhp|search|#|$|\?)(?:.(?!&tbm=))*$/,
			mutationTitle: true,
			engineList: 'web',
			style: '\
			border-bottom: 1px solid #E5E5E5;\
		...
		{
			name: "nyaa.se",
			url: /^https?:\/\/www\.nyaa\.se\/\?page=search/,
			engineList: "download",
			enabled: true,
			style: "\
			margin:0 auto;\
		...
		{
			name: "nyaa.eu",
			url: /^https?:\/\/www\.nyaa\.eu\/\?page=search/,
			engineList: "download",
			enabled: true,
			style: "\
			margin:0 auto;\
		...
		},
		{
			...
			url: /^https?:\/\/sukebei\.nyaa\.se\/\?page=search/,
			engineList: "download",
			enabled: true,
			style: "\
			margin:0 auto;\
		...
		{
			name: "sukebei.nyaa.eu",
			url: /^https?:\/\/sukebei\.nyaa\.eu\/\?page=search/,
			engineList: "download",
			enabled: true,
			style: "\
			margin:0 auto;\
