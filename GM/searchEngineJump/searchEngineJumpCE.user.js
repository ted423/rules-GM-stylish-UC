...
(function() {
	...
	var rules = [ //根据规则把搜索引擎列表插入到指定网站		
		...
		// 下载
		{
			...
			name: "种子狗",
			url: /^https?:\/\/www\.(zhongzigou3\.com\/so\/)|(yunbocili\.org\/list\/)/,
			engineList: "download",
			keyword: ".row.hidden-xs em",
		}, {
			name: "Torrentz2",
			...
			name: "RARBG",
			url: /^https?:\/\/(rarbgproxy\.org|rarbg\.is)\/torrents\.php\?search/,
			engineList: "download",
		}, {
			name: "BTSOW",
	...
	function addContainer(iTarget, iInput) {
		function parseDataStr(str) { //转换文本数据
			...
			function parseLine(container, line, isCurrent) {
				...
				for (var i = 0; i < line.length; i++) {
					...
					var engine = {};
					if (line[i].indexOf("%q") != -1) {
						engine.method = "POST";
						var two = line[i].split("%q");
						line[i] = two[0];
		...
		function getaPattern(arr,engine) {
				...
				engine.url = arr[1];
				if(engine.method == "POST") engine.url = engine.url.replace("%q","");
				...
				if (engine.method && engine.method.toUpperCase() == "POST") {
					var f = wrapToHide(getPostFormHTML(engine.url, engine.args, prefs.openInNewTab));//engine.args待研究
					a = a.replace("$form$", f);
		...
		function mousedownhandler(e) {
			...
			value = value.replace("#","%23");
				...
				for (var i = 0; i < inputs.length; i++) inputs[i].value = inputs[i].value.replace(/%q/g, value); //// 替换"全部"关键词
			}
		}
	}
